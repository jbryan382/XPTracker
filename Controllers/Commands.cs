...
namespace XPTracker.Controllers
{
  public class Commands : ModuleBase<SocketCommandContext>
  {
    ...
    public async Task DescribeMonsterAsync(string monster, string stat = "keys")
    {
      ...
      var client = new HttpClient();
      var body = await client.GetAsync(url);
      var bodyContent = body.Content.ReadAsStringAsync().Result;
      var jsonDoc = JsonDocument.Parse(bodyContent);
      var root = jsonDoc.RootElement;
      string keyFinderRegex = "\"([^\"]*)\":";
      MatchCollection matches = Regex.Matches(bodyContent, keyFinderRegex);
      var listOfKeys = new List<string>();
      foreach (Match match in matches)
      {
        foreach (Capture capture in match.Captures)
        {
          var monsterKey = capture.Value;
          listOfKeys.Add(monsterKey);
        }
      }
